...
#             self.episode_count += 1
#             if self.episode_count % 10 == 0:
#                 env = self.training_env.envs[0].env  # Unwrapped single env
#                 coverage = np.sum(env.visited) / (env.grid_size ** 2)
#                 self.metrics.log_episode(
#                     episode=self.episode_count,
#                     coverage=coverage,
...
#     raw_env = vec_env.envs[0].env  # Access the raw Gym env
...
class TrainingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.locals.get("dones", [False])[0]:
            self.episode_count += 1
            if self.episode_count % 10 == 0:
                env = self.training_env.envs[0].env  # Get original env
                coverage = np.sum(env.visited) / (env.grid_size ** 2)
                steps = env.step_count
                self.metrics.log_episode(
                    episode=self.episode_count,
                ...
                if self.verbose:
                    print(f"📘 Episode {self.episode_count}: Coverage={coverage:.2f}, Steps={steps}")
        return True
...
def train_model(total_timesteps=50000, model_name="ppo_MultiUAVEnv"):
    ...
    vec_env = make_vec_env(lambda: RealTimeMultiUAVEnv(grid_size=10, num_uavs=3), n_envs=1)
    raw_env = vec_env.envs[0].env  # Unwrapped single environment
    ...
    steps = 0
    while not done[0] and steps < 200:
        action, _ = model.predict(obs, deterministic=True)
        obs, reward, done, info = vec_env.step(action)
        steps += 1
