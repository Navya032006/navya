...
def test_model(model_path="models/ppo_MultiUAVEnv.zip", episodes=5):
    ...
    print(f"🚁 Testing over {episodes} episodes...")
    ...
    for episode in range(episodes):
        ...
        csv_data = env.generate_csv_data(f"data/test_episode_{episode + 1}.csv")
        print(f"  📁 CSV saved with {len(csv_data)} UAVs")

    metrics.save_metrics("data/test_metrics.csv")
    print("\n✅ All test episodes complete! Results saved to `data/test_metrics.csv`")
    return metrics


def test_real_time_performance():
    """Test real-time model inference speed."""
    print("\n⚡ Real-time inference test starting...")
    env = RealTimeMultiUAVEnv(grid_size=8, num_uavs=2)
    ...
    total_time = time.time() - start_time
    print(f"\n🚀 Real-time test completed in {total_time:.2f}s")
    print(f"⏱️ Average step time: {total_time / step_count:.4f}s")
